<script lang="ts">
	...
	async function submit() {
		...
		try {
			const url = getBaseAPIUrl() + '/v1/articles/' + data.article.id;
	...
	onMount(async () => {
		...
		const contributorId = data.article.contributor;
		...
		$articleRequest.content = data.article.content;
		$articleRequest.contributor = data.article.contributor;
		$articleRequest.description = data.article.description;
		$articleRequest.tags = data.article.tags;
		$articleRequest.title = data.article.title;

		if (data.article.external_url) {
			$articleRequest.external_url = data.article.external_url.String;
		}
	});
</script>
